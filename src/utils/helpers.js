...
/** Pulls the next value from the global auto-barcode sequence, encodes it,
 *  and hands back a value that is verified free ACROSS EVERY ACCOUNT
 *  before being returned — never merely within the current company.
 *
 *  The sequence guarantees two auto-generated values never repeat EACH
 *  OTHER, but it says nothing about a value someone already manually
 *  typed or scanned in (any company) — a real manufacturer barcode could
 *  coincidentally equal a not-yet-issued sequence value. So each
 *  candidate is checked against the full `products` table (no company
 *  filter, no barcode-shape filter — literally "does this value exist
 *  anywhere") and, on a collision, discarded in favor of the next
 *  sequence value. `products_autogenerated_barcode_unique` (migration
 *  023) is the DB-level backstop for the rare concurrent-request race;
 *  this loop is what makes retrying the normal, expected path rather
 *  than an error case.
 *
 *  Accepts an optional transaction so callers that create the product
 *  inside a `db.transaction()` can keep both the sequence pull and the
 *  existence check in the same transaction. */
async function nextAutoBarcode(trx) {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    const candidate = buildAutoBarcode(Number(rows[0].seq))

    // Global check — every account, regardless of how that row's own
    // barcode was originally entered.
    const collision = await runner('products').where({ barcode: candidate }).first('id')
