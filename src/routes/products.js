...
router.post('/', async (req, res, next) => {
  try {
    ...
    // ── Barcode: which validation rule applies is decided HERE, by which
    // branch runs — never by inspecting the resulting value's shape.
    //
    //   • A value present in the request means the user typed or scanned
    //     it (a real manufacturer/shop barcode) → validate uniqueness
    //     only within the CURRENT account. Another account already using
    //     the exact same value is expected and allowed.
    //   • No value present → the system generates one → validate/reserve
    //     it GLOBALLY, across every account, so it can never collide with
    //     anything anyone else auto-generates or types in later.
    const cleanBarcode = barcode?.toString().trim() || null
    let finalBarcode
    let barcodeAutoGenerated
    ...
    if (cleanBarcode) {
      ...
      // Auto-generate from the global product_auto_barcode_seq (migration
      // 021) — NOT from item_code's per-company counter. item_code repeats
      // across companies (every company's first product is MED-001), so
      // deriving the barcode from it let two companies generate the
      // identical barcode. nextAutoBarcode() also verifies the candidate
      // against every account before returning it (see utils/helpers.js).
      finalBarcode = await nextAutoBarcode()
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    if (updates.barcode !== undefined) {
      // Editing a product only ever exposes a manual barcode field — there
      // is no "auto-generate" action here — so any submitted change
      // (including clearing it) is explicitly a manual entry. Set the flag
      // directly rather than leaving whatever it was set to at creation.
      updates.barcode_auto_generated = false

      if (updates.barcode) {
        // Manual entry → uniqueness checked only within the current
        // account, excluding this product itself. Another account already
        // using the same value is allowed and must not be rejected.
        const dupe = await db('products')
